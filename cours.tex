...
est la perte du satelliste Mars Climate Orbiter (le coût de la mission était de 330 millions de dollars) 
...
non-SI alors que la NASA travaillait en SI. Il y a donc eu une très gossière erreur dans le calcul
...
exemple, les thermomètres consitués d'un tube en verre gradué 
...
l'aiguille et permet la lecture d'une température. Une autre technique consiste à utiliser le fait que différents matérieaux 
...
\end{center}
\end{figure} 
Finalement mentionnons également les thermomètres à infrarouge qui sont basés sur la mesure du rayonnement lumineux (mais pas forcément visible à l'oeil nu)
émis par tout corps. La ``couleur'' de ce rayonnement dépend de son énergie. Ces thermomètres fonctionnent donc en mesurant 
l'énergie émise par un corps. Ces thermomètres ont l'avantage de pouvoir mesurer la température à distance (pas besoin d'avoir un contact phyique avec la source) contrairement aux deux sortes de thermomètres décrits précédemment. Ils sont utilisés pour mesurer
...
et ne pas changer avec les conditions athmosphériques. 
...
atteinte par une mixture frigorifique (un mélange de galce d'eau et de chlorure d'ammonium dont la température se staibilise toute seule)
...
\item Calculer la température où la température en degrés Cellsius est la même qu'en degrés Fahrenheit ($T_C=T_F$).
\end{enumerate}
\end{exercice}
...
ont fera de même avec un bain contenant un mélange de vapeur et d'eau liquide. L'état du themomètre sera la température 100. 
Finalement il suffit de faire 99 graduations entre ces deux températures et le thermomètre est calibré (on peut rajouter des grauation de même taille au delà de l'intervalle 0-100). Néanmoins, chaque thermomètre a ses limitations en termes des températures qu'il peut effectivemnet mesurer
...
sa température de fusion. De même un thermomètre en spirale ne peux pas mesurer la tepérature au delà de 
...
nous pouvons décrire l'augmentation (ou diminution) du volume dû à l'augemtation (ou diminution) de la température.
...
\end{equation}
où $\Delta V=V-V_0$ et $\beta$ est le coefficient de dilatation volumique (encore une fois supposé constant). Lorsque le matériaux est dit isotrope (c'est-à-dire qu'il a les même propriétés dans toutes les directions) les coefficient de dilataion volumiques et linaires sont approximativement reliés par
\begin{equation}
\beta=3\alpha.
\end{equation}
...
\end{equation}
On constate que pour $\beta>0$ le volume va augmenter lorsqu'on chauffe (pour presque tous les méteriaux courant $\beta>0$ sauf pour l'eau qui pour certaines températures voit son coefficient de dilatation devenir négatif, voir Figure\ref{fig_alpha_eau}) alors que pour $\beta<0$ le volume va diminuer lorsqu'on chauffe.
\begin{figure}
\begin{center}
...
\rho=\frac{m}{V},
\end{equation}
où $m$ est la masse de l'ojet. La masse ne changeant pas avec la température (contrairement au volume), en substituant l'équation~\eqref{eq_V_T} dans l'équation ci-dessus, on obtient
\begin{equation}
